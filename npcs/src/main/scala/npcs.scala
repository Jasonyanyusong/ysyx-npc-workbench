...
*
* YSYX-NPCS is licensed under Mulan PSL v2.
...
class npc extends Module{
    ...
    val npcs_IFU = Module(new IFU)
    npcs_IFU.io.IFU_I_inst := io.NPC_getInst
    npcs_IFU.io.IFU_I_PC := PC
    ...
    val npcs_IDU = Module(new IDU)
    npcs_IDU.io.IDU_I_inst := npcs_IFU.io.IFU_I_inst
    //npcs_IDU.io.IDU_I_src1 := GPR_read(npcs_IDU.io.IDU_O_rs1)
    //npcs_IDU.io.IDU_I_src2 := GPR_read(npcs_IDU.io.IDU_O_rs2)
    val npcs_src1 = GPR_read(npcs_IDU.io.IDU_O_rs1)
    val npcs_src2 = GPR_read(npcs_IDU.io.IDU_O_rs2)
    io.NPC_halt := npcs_IDU.io.IDU_O_halt

    // Step III: ALU execution
    val npcs_EXU = Module(new EXU)
    npcs_EXU.io.EXU_I_src1 := npcs_src1
    npcs_EXU.io.EXU_I_src2 := npcs_src2
    npcs_EXU.io.EXU_I_imm := npcs_IDU.io.IDU_O_imm
    npcs_EXU.io.EXU_I_opcode := npcs_IDU.io.IDU_O_EXUopcode
    npcs_EXU.io.EXU_I_currentPC := PC

    // Step IV: LSU execution (we access memory using Verilator, NEMU and AM's debug interfaces)
    val npcs_LSU = Module(new LSU)
    npcs_LSU.io.LSU_I_src1 := npcs_EXU.io.EXU_O_result // LSU will get src1 + imm from EXU's result
    npcs_LSU.io.LSU_I_src2 := npcs_src2
    npcs_LSU.io.LSU_I_ModifyMem := npcs_IDU.io.IDU_O_ModifyMem
    npcs_LSU.io.LSU_I_opcode := npcs_IDU.io.IDU_O_LSUopcode
    io.NPC_LSU_O_len := npcs_LSU.io.LSU_O_len
    io.NPC_LSU_O_accessMem := npcs_LSU.io.LSU_O_accessMem
    io.NPC_LSU_O_memRW := npcs_LSU.io.LSU_O_memRW
    io.NPC_LSU_O_memAddr := npcs_LSU.io.LSU_O_memAddr
    io.NPC_LSU_O_memW := npcs_LSU.io.LSU_O_memW
    npcs_LSU.io.LSU_I_memR := io.NPC_LSU_I_memR

    // Step V: Write back data to a GPR and PC
    val npcs_WBU = Module(new WBU)
    npcs_WBU.io.WBU_I_LSUenable := !(npcs_IDU.io.IDU_O_LSUopcode === "b000000".U)
    npcs_WBU.io.WBU_I_EXUresult := npcs_EXU.io.EXU_O_result
    npcs_WBU.io.WBU_I_LSUresult := npcs_LSU.io.LSU_O_result
    npcs_WBU.io.WBU_I_rd := npcs_IDU.io.IDU_O_rd
    io.NPC_GPRchanged := npcs_IDU.io.IDU_O_GPRneedWriteBack
    GPR(npcs_WBU.io.WBU_O_GPRidx) := Mux(npcs_IDU.io.IDU_O_GPRneedWriteBack, npcs_WBU.io.WBU_O_GPRWriteBack, GPR_read(npcs_WBU.io.WBU_O_GPRidx))
    GPR(0) := 0.U(64.W)
    PC := npcs_EXU.io.EXU_O_dynamicNPC
