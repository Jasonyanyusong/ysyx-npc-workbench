    ...
    for (i = 0; i < NR_REGEX; i ++) {
      if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 && pmatch.rm_so == 0) {
        ...
        switch (rules[i].token_type) {
          ...
          default:                   {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = rules[i].token_type;  strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  break;}
          case TK_EQ:                {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_EQ;                strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  break;}
          case TK_NEQ:               {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_NEQ;               strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  break;}
          case TK_NOT:               {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_NOT;               strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  break;}
          case TK_AND:               {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_AND;               strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  break;}
          case TK_OR:                {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_OR;                strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  break;}
          case TK_POINTER:           {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_POINTER;           strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  break;}
          case TK_NUMBER:            {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_NUMBER;            strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  break;}
          case TK_REGISTER:          {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_REGISTER;          strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  break;}
          case TK_MARK:              {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_MARK;              strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  break;}
          case TK_LEFT_PARENTHESES:  {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_LEFT_PARENTHESES;  strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  break;}
          case TK_RIGHT_PARENTHESES: {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_RIGHT_PARENTHESES; strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  break;}
          case TK_DIVIDE:            {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_DIVIDE;            strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  break;}
          case TK_HEXNUMBER:         {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_HEXNUMBER;         strncpy(tokens[nr_token].str, substr_start, substr_len);  int convert_to_int;       sscanf(tokens[nr_token].str, "%x", &convert_to_int);  sprintf(tokens[nr_token].str, "%d", convert_to_int);  tokens[nr_token].type = TK_NUMBER;  break;}
          case TK_MULTIPLY:          {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_MULTIPLY;          strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  if((tokens[nr_token - 1].type != TK_NUMBER && tokens[nr_token - 1].type != TK_HEXNUMBER) || nr_token == 0){tokens[nr_token].type = TK_POINTER;}  nr_token = nr_token + 1;  break;}
          case TK_PLUS:              {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_PLUS;              strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  if(tokens[nr_token - 1].type != TK_NUMBER && tokens[nr_token - 1].type != TK_HEXNUMBER){tokens[nr_token].type = TK_POSITIVESIGN;}  break;}
          case TK_MINUS:             {memset(tokens[nr_token].str,0,sizeof(tokens[nr_token].str));  tokens[nr_token].type = TK_MINUS;             strncpy(tokens[nr_token].str, substr_start, substr_len);  nr_token = nr_token + 1;  if(tokens[nr_token - 1].type != TK_NUMBER && tokens[nr_token - 1].type != TK_HEXNUMBER){tokens[nr_token].type = TK_NEGATIVESIGN;}  break;}
        }
        break;
      }
    }
...
void give_priority_no_parentheses()
{
  ...
  for(int i = 1; i < nr_operator_token - 1; i = i + 1){
    if(operator_tokens[i].priority > local_highest_priority)
      local_highest_priority = operator_tokens[i].priority;
  }
  if(local_highest_priority < 0 || !check_parentheses_valid()){assert(0);}
  int give_priority_no_parentheses_parentheses_level = 0;
  for(int i = 0; i < nr_operator_token; i = i + 1){
    if(operator_tokens[i].token_type == TK_LEFT_PARENTHESES)
      give_priority_no_parentheses_parentheses_level = give_priority_no_parentheses_parentheses_level + 1;
    if(operator_tokens[i].token_type == TK_RIGHT_PARENTHESES)
      give_priority_no_parentheses_parentheses_level = give_priority_no_parentheses_parentheses_level - 1;
    if(operator_tokens[i].token_type != TK_LEFT_PARENTHESES && operator_tokens[i].token_type != TK_RIGHT_PARENTHESES){
      operator_tokens[i].priority = operator_tokens[i].priority + give_priority_no_parentheses_parentheses_level * local_highest_priority;
      operator_tokens_no_parentheses[nr_operator_tokens_no_parentheses].position = operator_tokens[i].position;
      operator_tokens_no_parentheses[nr_operator_tokens_no_parentheses].priority_level = operator_tokens[i].priority;
      operator_tokens_no_parentheses[nr_operator_tokens_no_parentheses].regex = operator_tokens[i].regex;
      operator_tokens_no_parentheses[nr_operator_tokens_no_parentheses].token_type = operator_tokens[i].token_type;
...
void give_sub_priority()
{
  ...
  for(int i = 0; i < nr_operator_tokens_no_parentheses; i = i + 1){
    if(operator_tokens_no_parentheses[i].priority_level > global_highest_priority)
      global_highest_priority = operator_tokens_no_parentheses[i].priority_level;
  }
  for(int i = global_highest_priority; i > 0; i = i - 1){
    int current_processing_local_priority = 0;
    for(int j = 0; j < nr_operator_tokens_no_parentheses; j = j + 1){
      if(operator_tokens_no_parentheses[j].priority_level == i)
        current_processing_local_priority = current_processing_local_priority + 1;
    }
    for(int j = 0; j < nr_operator_tokens_no_parentheses; j = j + 1){
      if(operator_tokens_no_parentheses[j].priority_level == i){
        operator_tokens_no_parentheses[j].sub_priority_level = current_processing_local_priority;
        current_processing_local_priority = current_processing_local_priority - 1;
      }
    }
  }
...
int process_add(int add_operator_index)            {return atoi(tokens[add_operator_index - 1].str)      + atoi(tokens[add_operator_index + 1].str);       }
int process_minus(int minus_operator_index)        {return atoi(tokens[minus_operator_index - 1].str)    - atoi(tokens[minus_operator_index + 1].str);     }
int process_multiply(int multiply_operator_index)  {return atoi(tokens[multiply_operator_index - 1].str) * atoi(tokens[multiply_operator_index + 1].str);  }
int process_devide(int devide_operator_index)      {return atoi(tokens[devide_operator_index - 1].str)   / atoi(tokens[devide_operator_index + 1].str);    }
int process_mod(int mod_operator_index)            {return atoi(tokens[mod_operator_index - 1].str)      % atoi(tokens[mod_operator_index + 1].str);       }
int process_equal(int equal_operator_index)        {return atoi(tokens[equal_operator_index - 1].str) == atoi(tokens[equal_operator_index + 1].str);       }
int process_not_equal(int not_equal_operator_index){return !process_equal(not_equal_operator_index);}
int process_and(int and_operator_index)            {return atoi(tokens[and_operator_index - 1].str)      & atoi(tokens[and_operator_index + 1].str);       }
int process_or(int or_operator_index)              {return atoi(tokens[or_operator_index - 1].str) | atoi(tokens[or_operator_index + 1].str);              }
int process_exor(int exor_operator_index)          {return atoi(tokens[exor_operator_index - 1].str) ^ atoi(tokens[exor_operator_index + 1].str);          }
int process_not(int not_operator_index)            {return ~ atoi(tokens[not_operator_index + 1].str);}
int process_positive_sign(int positive_sign_operator_index){return 1 * atoi(tokens[positive_sign_operator_index + 1].str);}
int process_negative_sign(int negative_sign_operator_index){return -1 * atoi(tokens[negative_sign_operator_index + 1].str);}
int process_pointer_dereference(int pointer_dereference_index){bool pointer_dereference_success = false; return isa_reg_str2val(tokens[pointer_dereference_index + 1].str + 1, &pointer_dereference_success);}
...
bool check_parentheses_valid()
{
  ...
  for(int i = 0; i < nr_token; i = i + 1){
    ...
    if(check_parentheses_valid_left_parentheses_count < check_parentheses_valid_right_parentheses_count < 0) return false;
  }
  return true;
}
...
void process_operator_token(){
  ...
  nr_operator_token = 0;
  for(int i = 0; i < nr_token; i = i + 1){
    if(tokens[i].type != TK_NUMBER && tokens[i].type != TK_HEXNUMBER){
      operator_tokens[current_index_of_operator_tokens].position = i;
      operator_tokens[current_index_of_operator_tokens].token_type = tokens[i].type;
      operator_tokens[current_index_of_operator_tokens].regex = tokens[i].str;
