...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, IFDEF(CONFIG_ShowInstName, printf("JAL\n"));    R(rd) = s -> pc + 4; s -> dnpc = s -> pc + imm; IFDEF(CONFIG_FunctionTrace, ftrace_write(false, s -> pc, s -> dnpc)));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, IFDEF(CONFIG_ShowInstName, printf("JALR\n"));   R(rd) = s -> pc + 4; s -> dnpc = (src1 + imm) & ~ 1; IFDEF(CONFIG_FunctionTrace, ftrace_write(!(rd == 0 && rs1 == 0 && imm == 0)), s -> pc, s -> dnpc));
